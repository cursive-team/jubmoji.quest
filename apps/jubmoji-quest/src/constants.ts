const GOOGLE_WALLET_ISSUER_ID = "3388000000022248072";

export const URLS = {
  GITHUB: "https://github.com/jubmoji/jubmoji.quest",
  PSE_DEV: "https://pse.dev",
  ZUCONNECT_TELEGRAM: "https://t.me/c/1938681868/1915",
};

export const MIN_SWIPE_DISTANCE = 50;

export const HUNT_TEAM_JUBMOJI_PUBKEY_INDICES = [
  61, 163, 117, 126, 80, 40, 104, 192, 116, 32,
];

export const HUNT_COLLECTION_JUBMOJI_PUBKEY_INDICES = [
  44, 133, 38, 127, 98, 109, 103, 69, 28, 45,
];

export const GOOGLE_PASS_CREDENTIALS = {
  type: "service_account",
  project_id: "self-1590553424634",
  private_key_id: "594f80a47fa4286fd772f58c5f7bd5ec21d48d51",
  private_key:
    "-----BEGIN PRIVATE KEY-----\\n-----END PRIVATE KEY-----\n",
  client_email: "project-vinyl@self-1590553424634.iam.gserviceaccount.com",
  client_id: "103146753565011709218",
  auth_uri: "https://accounts.google.com/o/oauth2/auth",
  token_uri: "https://oauth2.googleapis.com/token",
  auth_provider_x509_cert_url: "https://www.googleapis.com/oauth2/v1/certs",
  client_x509_cert_url:
    "https://www.googleapis.com/robot/v1/metadata/x509/project-vinyl%40self-1590553424634.iam.gserviceaccount.com",
  universe_domain: "googleapis.com",
};

export const APP_CONFIG = {
  APP_NAME: "Jubmojis",
  APP_SLUG: "Jubmojis Backup",
  WEBSITE_URI: "http://jubmoji.quest",
  EVENT_NAME: "Devconnect",
  COLLECTION_SIZE: 201,
  LOCATION: "Istanbul",
  COUNTRY: "Turkey",
  YEAR: "2023",
  RECOVERY_URL: (collection: string) =>
    `https://jubmoji.quest/recover#?collection=${encodeURIComponent(
      collection
    )}`,
};

// Google Wallet Object Constants
export const GOOGLE_WALLET = {
  ISSUER_ID: GOOGLE_WALLET_ISSUER_ID,
  CLASS_ID: `${GOOGLE_WALLET_ISSUER_ID}.jubmoji_pass_3`,
  OBJECT_ID: (serial: string) => `${GOOGLE_WALLET_ISSUER_ID}.${serial}`,
  HEX_BACKGROUND_COLOR: "#1E1E1F", // Hex color code
  LOGO_URI: "https://i.ibb.co/370zVNG/logo-2x.png",
  HERO_IMAGE_URI: "https://i.ibb.co/Ht2S11B/strip.png",
  /**
   * It's okay to expose our Google Wallet API private key here because the
   * "useful" information in the passes are the signatures from the cards,
   * which cannot be forged. Without those signatures, no valid ZKPs can be made.
   * This is a necessary hack to avoid sending the user's private signatures
   * to the server to get signed, which is usually how passes are handled.
   */
  PRIVATE_KEY:
    "-----BEGIN PRIVATE KEY-----\\n-----END PRIVATE KEY-----\n",
};
